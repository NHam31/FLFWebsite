...
import { Eye, EyeOff, UserPlus, CheckCircle, FileText, IdCard } from "lucide-react";
...
export default function SignUp() {
  ...
  const handleFileUpload = async (file: File, type: "attestation" | "idCard") => {
    ...
    if (!allowedTypes.includes(file.type)) {
      toast.error("نوع الملف غير مسموح به. الملفات المقبولة: PDF, JPG, PNG");
      return;
    }

    if (type === "attestation") setUploadingAttestation(true);
    ...
    reader.onloadend = async () => {
      ...
      try {
        ...
        if (type === "attestation") {
          ...
          setFormData((prev) => ({ ...prev, idCardUrl: result.fileRef }));
          toast.success("تم رفع بطاقة التعريف بنجاح");
        }
      } catch {
        toast.error("حدث خطأ أثناء رفع الملف");
      } finally {
        if (type === "attestation") setUploadingAttestation(false);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (!formData.studyStatus) {
      toast.error("يرجى اختيار حالة الدراسة");
      return;
    }
        ...
        >
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
            <div className="text-center mb-8">
              ...
              <h1 className="text-2xl font-bold text-gray-900">التسجيل في المؤسسة</h1>
              <p className="text-gray-500 text-sm mt-2">أنشئ حسابك للانضمام إلى مؤسسة أطر الغد</p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              {/* Study status */}
              <div>
                <Label htmlFor="studyStatus">الحالة الدراسية الحالية *</Label>
                <Select
                  value={formData.studyStatus}
                  onValueChange={(v) => setFormData({ ...formData, studyStatus: v })}
                >
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="master_student">طالب ماجستر</SelectItem>
                    <SelectItem value="phd_student">طالب دكتوراه</SelectItem>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="hidden"
                    onChange={(e) => e.target.files?.[0] && handleFileUpload(e.target.files[0], "attestation")}
                  />
                  <Button
                    type="button"
                    variant="outline"
                    ...
                    className={`w-full mt-1 ${formData.attestationUrl ? "border-green-500 text-green-600" : ""}`}
                  >
                    <FileText className="w-4 h-4 mr-2" />
                    {uploadingAttestation
                      ? "جاري الرفع..."
                      : formData.attestationUrl
                        ? "تم الرفع"
                        : "رفع الشهادة"}
                  </Button>
                </div>
                <div>
                  ...
                  <Button
                    ...
                    className={`w-full mt-1 ${formData.idCardUrl ? "border-green-500 text-green-600" : ""}`}
                  >
                ...
                <div className="flex gap-6">
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      ...
                      onChange={() => setFormData({ ...formData, isAmbassador: true })}
                      className="w-4 h-4 text-[#4A9B8E]"
                    />
                    <span className="text-sm">نعم، أنا سفير</span>
                  </label>
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input
                      type="radio"
                      name="ambassador"
                      checked={!formData.isAmbassador}
                      onChange={() => setFormData({ ...formData, isAmbassador: false })}
                      className="w-4 h-4 text-[#4A9B8E]"
                    />
                    <span className="text-sm">لا، أنا مرشح جديد</span>
                  </label>
                </div>
              </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <Label htmlFor="newsletter" className="text-sm font-normal text-gray-600 leading-relaxed cursor-pointer">
                  أوافق على تلقي الأخبار والتحديثات الخاصة بالمؤسسة عبر البريد الإلكتروني
                </Label>
              </div>
            ...
            <p className="text-center text-sm text-gray-500 mt-6">
              لديك حساب بالفعل؟{" "}
              <Link to="/signin" className="text-[#4A9B8E] hover:text-[#3D7A6F] font-medium">
                تسجيل الدخول
              </Link>
            </p>
          </div>
